"""Anomaly Batch 2 -- Tests B, C, D (pre-registered, frozen spec) -- CORRECTED COST MODEL.

This is a COPY of anomaly_batch2.py, created 2026-07-27 as a research re-test
after the 2026-07-26/27 cost-model audit. anomaly_batch2.py and its results
file (anomaly_batch2_results.txt) are left untouched as the historical
artifact -- this file exists ONLY to answer "does the frozen verdict change
once the cost bug is fixed", not to replace the original run.

ONE functional change vs. the original, isolated to the fee computation in
USim.run():
    OLD (buggy):  fees = sum(zerodha_charges.calculate_charges(buy_v, sell_v,
                            is_intraday=False).values())
    NEW (fixed):  fees = zerodha_charges.calculate_charges(buy_v, sell_v,
                            is_intraday=False)['total']
Why: calculate_charges() returns a dict whose 'total' key is already the sum
of the other six charge components (brokerage, stt, exchange, sebi, dp, gst,
stamp_duty). summing .values() therefore adds 'total' on top of its own
components -- an exact 2x double-count of every charge on every fill. This
was the (a) bug named in the audit. config.py's constants (b) were already
corrected in a prior commit (exchange 0.00297%, intraday stamp duty 0.003%,
GST base including SEBI, DP charge on delivery sells) -- nothing to change
here, config.zerodha_charges is imported as-is.

ONE non-functional addition, needed only to run the (c) slippage sensitivity
sweep the audit asked for without hand-editing this file three times: SLIP
and OUT_FILE are read from environment variables ANOMALY_SLIP / ANOMALY_OUT
with defaults identical to the original (SLIP=0.002, same OUT_FILE name).
Calling the script with no environment overrides reproduces the original's
SLIP assumption exactly, with only the cost-computation bug fixed.

No pre-registered threshold, signal definition, split, benchmark, or engine
logic is touched. Whatever prints is the answer -- same as the original.
...
REUSES kite/research/universe_lab.py conventions verbatim where the spec says to:
  ...
  - USim: next-day-open fills, SLIP=0.002 (0.2%/side) by default, delivery charges
    via config.zerodha_charges(is_intraday=False), monthly rebalance (first trading day).
...
Usage: python -W ignore kite/research/anomaly_batch2_corrected.py
       ANOMALY_SLIP=0.0005 python -W ignore kite/research/anomaly_batch2_corrected.py
"""
import os
import sys
...
OUT_FILE = Path(os.environ.get('ANOMALY_OUT', str(ROOT / 'kite' / 'research' / 'anomaly_batch2_corrected_results.txt')))
SLIP = float(os.environ.get('ANOMALY_SLIP', '0.002'))
CAPITAL = 100_000
...
# ---------------------------------------------------------------------------
# USim -- copied from universe_lab.py, with the (a) cost-model bug fixed:
# fees now read the pre-summed 'total' key instead of summing the whole dict
# (which double-counted, since 'total' is itself the sum of the other six
# components). This is the ONLY line changed from anomaly_batch2.py's USim.
# ---------------------------------------------------------------------------
class USim:
...
def test_D(data, all_dates):
    ...
    log('  NOTE: Test D is out of scope for this corrected re-run (task asked for B and C only);')
    log('  its flat 0.25%/day assumption is untouched by the SLIP/charges fix and reproduced')
    log('  unchanged here purely so main() stays a faithful copy of the original script.')
...
def main():
    log('ANOMALY BATCH 2 -- Tests B, C, D -- CORRECTED COST MODEL (research re-test)')
    log(f'Spec: docs/superpowers/specs/2026-07-22-anomaly-batch2-design.md (thresholds FROZEN, unchanged)')
    log('Cost fix applied: fees = calculate_charges(...)[\'total\']  (was: sum(...values()),')
    log('which double-counted because the dict already contains a \'total\' key equal to the')
    log('sum of the other six components). config.py constants were already corrected')
    log('separately (exchange txn charge, intraday stamp duty, GST base, DP charge).')
    log(f'SLIP (per-side slippage) for THIS run = {SLIP * 100:.4f}%   [env ANOMALY_SLIP, default 0.002 = original assumption]')
    log('')
    data = load()
